...
def _build_bodies(outcome: dict) -> tuple[str, str, str]:
    ...
    papers = outcome.get("papers", 0)
    news = outcome.get("news", 0)

    subject = f"{keyword} · {run_type} · papers={papers} news={news}"
    text = (
        f"[SignalHub] keyword={keyword} run_type={run_type}\n"
        f"sources: arxiv={papers}, geeknews={news}\n"
    ...
    html = (
        ...
        f"arxiv: {papers} · geeknews: {news}</p>"
