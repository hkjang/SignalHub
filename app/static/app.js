...
$('#kw-list').addEventListener('click', async (e) => {
  ...
  if (e.target.classList.contains('kw-del')) {
    ...
    try {
      ...
      toast(`완료: 논문 ${r.papers} · 뉴스 ${r.news}`);
      loadKeywords();
...
async function openDetail(id) {
  try {
    ...
    const papersEl = $('#m-papers');
    const papers = (r.sources && r.sources.papers) || [];
    ...
    const newsEl = $('#m-news');
    const news = (r.sources && r.sources.news) || [];
...
$('#quick-run-btn').addEventListener('click', async () => {
  ...
  try {
    const r = await api('/run', { method: 'POST', body: JSON.stringify({ keyword: name }) });
    $('#quick-run-status').textContent = `완료 #${r.id} · 논문 ${r.papers} · 뉴스 ${r.news}`;
    loadDashboard();
  } catch (e) {
